...
public class BottomSheetBehaviorGoogleMapsLike<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public boolean onLayoutChild( CoordinatorLayout parent, V child, int layoutDirection ) {
        ...
        Log.i("bottom:","mState="+mState + " mAnchorPoint"+mAnchorPoint+" mParentHeight"+mParentHeight+" mMinOffset"+mMinOffset
        +" "+mMaxOffset);
        /**
         * New behavior
         */
    ...
    public void onNestedPreScroll( CoordinatorLayout coordinatorLayout, V child, View target, int dx, int dy, int[] consumed ) {
        ...
        else               //ed:canscroll 1能否向上滚 -1向下滚，true可以
        if ( dy < 0 ) { // Downward
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView( View child, int pointerId ) {
            if ( mState == STATE_DRAGGING ) {
                Log.i("bottom:","STATE_DRAGGING不允许");
                return false;
            }
            if ( mTouchingScrollingChild ) {
                Log.i("bottom:","mTouchingScrollingChild不允许");
                return false;
            }
            if ( mState == STATE_EXPANDED  &&  mActivePointerId == pointerId ) {
                ...
                if (scroll != null && ViewCompat.canScrollVertically(scroll, -1)) {
                    ...
                    Log.i("bottom:","canScrollVertically不允许");
                    return false;
                }
            }
            Log.i("bottom:",(mViewRef != null && mViewRef.get() == child)?"允许":"不允许");
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            Log.i("bottom:","top="+top+" dy="+dy+" mMinOffset"+mMinOffset+" mHideable="+String.valueOf(mHideable)+" mParentHeight"+mParentHeight);
