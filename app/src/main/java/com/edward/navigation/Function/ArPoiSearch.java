...
import com.edward.Reback.Employee;
import com.edward.Reback.ListviewWorker;
import com.edward.navigation.Adapter.SearchpoiAdapter;
import com.edward.navigation.Control.Data;
import com.edward.navigation.Control.Util;
import com.edward.navigation.Model.SearchpoiEntity;
                       ...
                       // initnewview(poiItems.size());
                        ...
                        for (int i = 0; i < poiItems.size(); i++) {
                            ...
                            Data.SearchpoiList.add(new SearchpoiEntity(MyResult.toString(),
                                    MyResult.getTypeDes(), (int) distance,mybearing,
                                    MyResult.getSnippet() != null ? MyResult.getAdName() + MyResult.getSnippet()
                                            : MyResult.getAdName() + MyResult.getBusinessArea() + MyResult.getDirection(),
                                    MyResult.getLatLonPoint()));
                            //snippet 片段，即poi点的详细位置数据，需要加上区地址好点
                            // 有时会返回为空，设置时，若为空则设置为其区地址
                            //之前才用poiItems.get(i).getdistance()，返回值会出现-1的情况，
                            //原因是距离过大。所以采用自己计算
